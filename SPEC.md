...
- MET (2026-09-22, piece 3; his word "let's build piece 3") — THE CHECK THAT RUNS IT. `page_from` proves a page is whole and local and cannot prove it WORKS, because nothing ran it: version 1 of the snake game called `clearInterval(game)` with no `game` declared and was saved. The page is now loaded in a browser with no window before a version is kept, and every uncaught error, rejected promise, failed load and `console.error` goes back to the Coder for ONE more try. **The browser is already on this machine and nothing is downloaded (RULE 4):** Edge first, Chrome as fallback, his ruling the same day. The page is SERVED from `http.server` on 127.0.0.1 with a catcher injected behind the doctype (ahead of the page's scripts, so it catches them; behind the doctype, so the check never tests a page in quirks mode), and the reported line numbers are corrected back to the Coder's OWN page. **It never blocks a save** (his ruling): a page that still errors is kept and the report says so, because `page_from` already refuses what is fatal and the fault that started this piece was a real error in a perfectly playable game; a check that could not run — no browser, a launch that failed, a page that never loaded — says that instead. Kept only if not worse: a repair that breaks in MORE places than the page it was asked to fix is refused, counted by the same machine that counted the first. `test_the_maker_runs_the_page_before_it_keeps_it` (33 strokes), proved by four reversals, and the fail-open half stroked with no browser at all — which is how this passes on a CI runner that has none.
...
BUILDPATH, "The order it goes next".
    ...
    THE LOOP -- his workflow direction of 2026-09-04, still unbuilt here
      ...
      2026-09-22   THE FIRST LAWFUL LOOP EXISTS, and it is the maker's check
                   (4.8): a node returned to, under all three of LAW_003's
                   bounds, proved. That law was sealed 2026-09-17 and nothing
                   had used it -- LAW_003's own diagnosis being that while
                   every cycle was refused by name, the lawful provable thing
    ...
    THE MAKER -- his vision of 2026-09-21; pieces 1 and 2 built the same day
      what it is   "make me a snake game" made, versioned and reported by the
                   ...
                   (MET 2026-09-22 -- and the estate's first lawful loop
                   under LAW_003). The order is BUILDPATH's, "The maker".
                   ALL THREE ARE BUILT; what is left is the test itself.
