...
VOCAB_BIAS = ("This is the manjuel estate: Steward, Manjuel, Jesster, Neiro, "
...
CPP_BIN_ENV = "MANJUEL_WHISPER_CLI"     # path to whisper-cli(.exe)
CPP_MODEL_ENV = "MANJUEL_WHISPER_GGML"  # path to ggml-*.bin
...
def local_model() -> tuple[str, str]:
    ...
    return "", (f"no local '{tag}' found (looked in: {searched}). "
                ...
                f"MANJUEL_WHISPER_MODEL to the model path itself.")
...
def _speech_cmd(body: str, voice: str | None = None):
    ...
    if kind == "sapi":
        ...
        script = (
            ...
            "$t = [IO.File]::ReadAllText($env:MANJUEL_SPEAK_FILE, "
            "[Text.Encoding]::UTF8); "
            "$s = New-Object System.Speech.Synthesis.SpeechSynthesizer; "
            "$v = $env:MANJUEL_SPEAK_VOICE; "
            "if ($v) { try { "
        ...
        env = dict(os.environ, MANJUEL_SPEAK_FILE=path,
                   MANJUEL_SPEAK_VOICE=(voice or ""))
